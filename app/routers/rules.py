# -- scripts imports --
from app.db_models import MissionDataRow, GeometryRow, MissionAssetRow, TeamRow, GenericRuleRow
   ...
   for geo in geos_data:
       ...
       if geo.id and not str(geo.id).startswith('g-'):
           ...
           if geo_row:
               ...
               geo_row.system_uuid = geo.system_uuid
...
def _update_geometry_to_team_mappings(rule_uuid, mission_id, geometry_ids, db: Session):
   # 1. Clear existing mappings for this rule
   db.query(MissionAssetRow).filter(MissionAssetRow.rule_uuid == rule_uuid).delete()

   # 2. Get the team (assuming there's one team for now as per current code)
   ...
   # 3. Add mappings
   if not geometry_ids:
       # If no geometries, add a single row with geometry_uuid=None
       db.add(MissionAssetRow(
           geometry_uuid=None,
           mission_uuid=mission_id,
           team_uuid=team.uuid,
           rule_uuid=rule_uuid,
           geometry_system_uuid=None
       ))
   else:
       # Fetch system_uuids for all geometries in one go for efficiency
       geos = db.query(GeometryRow.uuid, GeometryRow.system_uuid).filter(GeometryRow.uuid.in_(geometry_ids)).all()
       geo_system_map = {str(g.uuid): g.system_uuid for g in geos}

       # Add a row for each geometry
       for g_id in geometry_ids:
           db.add(MissionAssetRow(
               ...
               team_uuid=team.uuid,
               rule_uuid=rule_uuid,
               geometry_system_uuid=geo_system_map.get(str(g_id))
           ))
...
def _process_rule_creation(data: RuleCreate, db: Session):
   ...
   db.add(new_rule)
   db.flush()  # Generate the UUID
   _update_geometry_to_team_mappings(new_rule.uuid, data.rule.missionId, final_geo_ids, db)
...
def _process_rule_update(rule_id: str, data: RuleCreate, db: Session):
   ...
   _update_geometry_to_team_mappings(rule_id, data.rule.missionId, final_geo_ids, db)
...
def bulk_update_rules(items: List[RuleCreate], db: Session = Depends(get_db_session)):
   try:
       for data in items:
           _process_rule_update(data.rule.id, data, db)
       db.commit()
